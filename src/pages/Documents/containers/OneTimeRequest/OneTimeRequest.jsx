...
const DocsLabels = {
  onetimerequest: {
    ...
    typeDocs: 'onetimerequest',
    createDocs: DocsAPI.createOneTimeRequest ,
    files: 'onetimerequest__list_docs'
  }
};
...
const OneTimeRequestContainer = ({ type }) => {
  ...
  const loadDocuments = () => {
    const inputElement = document.querySelector(`input[name="CFV[1277067]"]`);
    if (inputElement) {
      const savedData = inputElement.value;
  ...
  const handleGenerateClientRequest  = async () => {
    ...
    const requestBody = {
      doc_type: "one_time_request",
      filename: "Заявка_разовая_с_клиентом_для.docx",
      amo_id: id,
  ...
  const handleUpdateDocuments = () => {
    const inputElement = document.querySelector(`input[name="CFV[1277067]"]`)
    if (inputElement) {
      updateInputValue('1277067', JSON.stringify(documents || []));
    }
  }
  ...
  return (
      <div>
        <Container>
          ...
          >
            <FilesContainer dataDocs={documents} name={'Заявка_разовая_с_клиентом_для.docx'}/>
          </Container>
...
export default OneTimeRequestContainer;
