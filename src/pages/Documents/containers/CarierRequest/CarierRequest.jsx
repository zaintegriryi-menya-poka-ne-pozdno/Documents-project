...
const DocsLabels = {
  carierrequest: {
    ...
    fields: [
      ...
      { nameId: 'dop_info',  id: 1277105, typeValue: 'text',  name: 'Дополнительная информация' },
    ],
    typeDocs: 'carierrequest',
    createDocs: DocsAPI.createCarierRequest,
    files: 'carierrequest__list_docs'
  }
};
...
const CarierRequestContainer = ({ type }) => {
  ...
  const loadDocuments = () => {
    const inputElement = document.querySelector(`input[name="CFV[1277093]"]`);
    if (inputElement) {
      const savedData = inputElement.value;
  ...
  const handleGenerateClientRequest  = async () => {
    ...
    const requestBody = {
      doc_type: "carier_request",
      filename: "Заявка_с_перевозчиком_для.docx",
      amo_id: id,
  ...
  const handleUpdateDocuments = () => {
    const inputElement = document.querySelector(`input[name="CFV[1277093]"]`)
    if (inputElement) {
      updateInputValue('1277093', JSON.stringify(documents || []));
    }
  }
  ...
  return (
      <div>
        <Container>
          ...
          >
            <FilesContainer dataDocs={documents} name={'Заявка_с_перевозчиком_для.docx'}/>
          </Container>
...
export default CarierRequestContainer;
